...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-1100 w-full h-16 bg-background/80 backdrop-blur-md border-b border-border/50 shadow-sm transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <nav className="hidden md:flex items-center gap-3">
            ...
            <Link
              href="/mapas"
              className={`${navLinkBase} bg-primary text-white border border-gray-300 rounded-4xl btn-hover-primary shadow-sm`}
            >
              Meus Mapas
            </Link>
      ...
      {/* MOBILE MENU */}
      <div
        className={`md:hidden absolute w-full bg-background/95 backdrop-blur-lg border-b border-border shadow-xl transition-all duration-300 ease-in-out origin-top ${
          isMenuOpen
      ...
      >
        <nav className="flex flex-col p-4 gap-3">
          ...
          <Link
            href="/mapas"
            onClick={() => setIsMenuOpen(false)}
            className={`${navLinkBase} bg-primary text-white hover:bg-gray-300 rounded-3xl py-4`}
          >
            Meus Mapas
          </Link>
        </nav>
